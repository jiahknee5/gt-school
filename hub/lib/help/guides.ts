...
export const GUIDES: Guide[] = [
  // ─────────────────────────────── Run the cadence ───────────────────────────────
  {
    ...
    steps: [
      {
        do: "Set the week-of selector and read the Executive Narrative (topline / working / stuck / decisions).",
        where: "Home / weekly context",
        result: "Everyone is anchored on the same week and the 5-minute exec recap.",
        href: "/",
        target: "tour-week-selector",
      },
      {
        do: "Scan the canonical scorecard: applicants, deposits, conversion, SLA, and handoffs this week vs last with status chips.",
        where: "Dashboard / scorecard",
        result: "Shared numbers, biggest movers, and red flags are obvious.",
        href: "/m/dashboard",
        target: "tour-dashboard-scorecard",
      },
      {
        do: "Walk each workstream Overview in turn: Grassroots, Content, Nurture, CRM Ops, Admissions, and Analytics.",
        where: "Module / overview",
        result: "Each owner reports from their live dashboard; goal pacing is visible.",
        href: "/m/grassroots",
        target: "tour-module-overview",
      },
      {
        do: "Close on open decisions: approve, reject, or request more info on each card.",
        where: "Decision Queue / active",
        result: "Decisions are recorded with a ruling and comment; submitters get notified.",
        href: "/m/decisions",
        target: "tour-decision-queue",
      },
    ],
    success: [
      "Every agenda item maps to a Hub screen, with no external docs.",
