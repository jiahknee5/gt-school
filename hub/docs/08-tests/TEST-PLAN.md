...
33 files, 414 passed, 0 todo. `tests/brief-usecases.test.ts`: all `covered`
...
- **Ask-the-Hub hardening**: provider-backed synthesis, deterministic fallback, graph trace nodes,
  `/dev/agents`, and eval rows are covered; add persisted DB ask-audit rows and a full live aggregate
  adapter after the pure demo path.
