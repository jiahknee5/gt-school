...
| **C2** | Navigation & IA | ✅ Pass (P1 grouping proposed) | `lib/modules.ts` is the single nav source; all 13 modules + Campaigns + Help + Developer render from it in the sidebar. Home is `/`; the rest are `/m/<slug>`. Every module header links back Home. **Module grouping** (the flat 13 → MECE categories + nav mapping) is worked out in [`IA-MODULE-ORGANIZATION.md`](IA-MODULE-ORGANIZATION.md) (the `gt-hub-ia-panel` audit) — additive, deferred P1. |
