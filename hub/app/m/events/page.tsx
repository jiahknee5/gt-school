          ...
          <div className="space-y-5">
              ...
              <>
                <Card title="Top type by attendance" note="Single metric definitions; figures compute purely from field_events.">
                  <div className="space-y-1">
                    {topTypes.map((t) => (
                      <div key={t.type} className="flex items-center justify-between border-b border-hairline py-1 text-[12px]">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Card title="Budget reconcile" note="Event spend is a component of a workstream actual — no new line.">
                    {Object.entries(spend).map(([ws, amt]) => (
                      <div key={ws} className="flex items-center justify-between border-b border-hairline py-1 text-[12px]">
            ...
            {activeTab === "tracker" && (
              <Card title="Event tracker" note={`${dupes.length} duplicate (name+date) flag(s) · ${stale.length} stale/incomplete. Required fields validated on write.`}>
                {dupes.length > 0 && (
                  <div className="mb-3 rounded-card border border-amber-soft bg-amber-soft p-2 text-[12px] text-amber">
                  ...
                  <table className="w-full min-w-[680px] border-collapse text-[13px]">
                    <thead>
                      <tr className="border-b border-hairline text-left text-[11px] uppercase tracking-wide text-label">
                        ...
                        <th className="py-2 pr-3 font-semibold">Attended</th>
                        <th className="py-2 pr-3 font-semibold">Consults</th>
                    ...
                    <tbody>
                      {events.map((e) => (
                        <tr key={e.id} className="border-b border-hairline">
                          <td className="py-2 pr-3 font-semibold text-ink">{e.name}</td>
                          <td className="py-2 pr-3 text-muted">{e.type.replace("_", " ")}</td>
                          <td className="py-2 pr-3 text-muted">{e.eventDate}</td>
                          <td className="mono num py-2 pr-3 text-muted">{fmt(e.rsvpCount)}</td>
                          <td className="mono num py-2 pr-3 text-muted">{fmt(e.attendance)}</td>
                          <td className="mono num py-2 pr-3 text-muted">{fmt(e.consultsBooked)}</td>
                          <td className="py-2"><Pill tone={e.status === "completed" ? "good" : e.status === "planning" ? "watch" : "neutral"}>{e.status}</Pill></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {activeTab === "calendar" && (
              <Card title="Calendar" note={`${gtCount} GT-organized events. Ambassador events (read-only, from Grassroots) are shown distinctly and never counted here. Type encoded by color + text/shape.`}>
                <div className="space-y-1">
                  {calendar.map((c) => (
                    <div key={c.id} className="flex items-center justify-between border-b border-hairline py-1 text-[12px]">
              ...
              <Card title="Priority event proposals" note="Submit emits exactly one Decision Queue row (idempotent). Operators submit but cannot view/act on the queue.">
                <div className="space-y-2">
                  {SEED_PROPOSALS.map((p) => (
                    <div key={p.id} className="rounded-card border border-hairline bg-canvas p-3">
                      ...
                      <p className="mt-1 text-[11px] text-muted">{p.type} · {p.proposedDate} · ask {usd(p.budgetAsk)} · {p.workstreamKey}</p>
                    </div>
                  ))}
                </div>
                {!canViewDecisionQueue(viewer.role) && (
                  <p className="mt-3 text-[12px] text-red">You can submit proposals but cannot view/act on the Decision Queue (submit-only).</p>
                )}
              </Card>
            )}
          </div>
