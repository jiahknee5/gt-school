  ...
  return (
    <section id={t.name} className="scroll-mt-6">
      <div className="flex flex-wrap items-baseline gap-2.5">
        <h2 className="mono text-[16px] font-bold text-ink">{t.name}</h2>
        ...
        {count !== null ? (
          <span className="num mono text-[11px] text-label">{count.toLocaleString()} rows</span>
        ) : null}
      </div>
      <p className="mt-1 text-[13px] font-medium text-slate">{t.title}</p>
      <p className="text-[12px] text-muted">
        <span className="mono text-label">Source of truth:</span> {t.sourceOfTruth}
      </p>
      <p className="mt-1.5 max-w-[760px] text-[12px] leading-relaxed text-muted">{t.why}</p>
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-hairline bg-side">
              <th className="mono px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-label">Field</th>
              <th className="mono px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-label">Type</th>
              <th className="mono px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-label">Description</th>
            </tr>
          </thead>
          <tbody>
            {t.fields.map((f) => (
              <tr key={f.name} className="border-b border-hairline last:border-0 align-top">
                <td className="mono px-3 py-2 text-[12px] font-medium text-ink">{f.name}</td>
                <td className="mono px-3 py-2 text-[11px] text-muted">{f.type}</td>
...
export default async function DataDictionary() {
  ...
  return (
    <div className="mx-auto max-w-[1180px] px-7 py-10">
      ...
      <div className="mt-8 lg:grid lg:grid-cols-[180px_minmax(0,1fr)] lg:gap-8">
        {/* sticky ToC */}
              ...
              <div key={zone}>
                ...
                <ul className="mt-1.5 space-y-0.5">
                  {tablesByZone(zone).map((t) => (
                    <li key={t.name}>
                      <a href={`#${t.name}`} className="mono text-[12px] text-muted hover:text-blue">
                        {t.name}
                      </a>
                    </li>
                  ))}
          ...
          {ZONES.map((zone) => (
            <div key={zone} className="space-y-8">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[12px] text-muted">{ZONE_META[zone].blurb}</span>
              </div>
              {tablesByZone(zone).map((t) => (
