  ...
  return (
    <div className="mx-auto max-w-[1180px] px-4 py-5">
      ...
      <div className="mt-4 rounded-card border border-hairline bg-surface p-3">
        <p className="mono text-[10px] font-semibold uppercase tracking-[0.1em] text-label">How to run</p>
        ...
        return (
          <section key={group.id} className="mt-7">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <p className="mono text-[10px] font-semibold uppercase tracking-[0.12em] text-gold">{group.label}</p>
              <span className="mono text-[10px] text-label">
                {gc.total} tests{gc.live > 0 && <span className="text-amber"> · {gc.live} live</span>}
              </span>
              <code className="mono ml-auto rounded-[5px] bg-fill px-1.5 py-0.5 text-[10px] text-slate">{group.script}</code>
            </div>
            <p className="mt-1 max-w-[820px] text-[11px] leading-snug text-muted">{group.domain}</p>
