...
function ProviderCard({ p }: { p: ProviderSummary }) {
  return (
    <Link
      ...
      className="group flex flex-col rounded-card border border-hairline bg-surface p-2.5 shadow-sm transition-colors hover:border-border"
    >
      ...
      {p.description ? (
        <p className="mt-1 line-clamp-2 text-[11px] leading-snug text-muted">{p.description}</p>
      ) : null}
      <div className="mono mt-1.5 flex gap-2 text-[11px] text-label">
        <span>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ...
              <div key={label} className="rounded-card border border-hairline bg-surface p-2.5">
                ...
                <div className="mt-0.5 font-serif text-[18px] font-bold text-ink">{value}</div>
              </div>
            ))}
          </div>
        ) : null}
        ...
        {tiers && tiers.other.length > 0 ? (
          <section className="mt-6">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[11px] text-muted">{tiers.other.length} sources</span>
            </div>
            <div className="mt-2.5 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {tiers.other.map((p) => (
                <ProviderCard key={p.slug} p={p} />
